...
class BERTBiLSTMNERSentimentModel(nn.Module):
    def __init__(self, num_ner_labels, num_sentiment_labels, hidden_size=256, num_lstm_layers=3):
        super(BERTBiLSTMNERSentimentModel, self).__init__()
        ...
        self.ner_attention = AttentionLayer(hidden_size * 2)
        self.sentiment_attention = AttentionLayer(self.bert.config.hidden_size)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        ner_attended = self.ner_attention(lstm_output)
        sentiment_attended = self.sentiment_attention(sequence_output)
        
        ner_output = self.ner_classifier(self.dropout(lstm_output))
...
if __name__ == "__main__":
    ...
    model = BERTBiLSTMNERSentimentModel(num_ner_labels=5, num_sentiment_labels=3, num_lstm_layers=3)
    ...
    plot_confusion_matrix(
        ...
        title='BERT-BiLSTM Sentiment Analysis Confusion Matrix'
    )
    ...
    plot_metrics_comparison(
        ...
        title='BERT-BiLSTM Performance Comparison'
    )
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.title('BERT-BiLSTM Training Progress')
    plt.savefig('./performance_improved/training_progress_BERT_BiLSTM.png')
    plt.show()
