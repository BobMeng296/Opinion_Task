...
class BERTBiLSTMMultiHeadNERSentimentModel(nn.Module):
    def __init__(self, num_ner_labels, num_sentiment_labels, hidden_size=256, num_lstm_layers=3, num_attention_heads=8):
        super(BERTBiLSTMMultiHeadNERSentimentModel, self).__init__()
        ...
        self.ner_attention = MultiHeadAttentionLayer(hidden_size * 2, num_attention_heads)
        self.sentiment_attention = MultiHeadAttentionLayer(self.bert.config.hidden_size, num_attention_heads)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        ner_attended = self.ner_attention(lstm_output, attention_mask)
        sentiment_attended = self.sentiment_attention(sequence_output, attention_mask)
        
        ner_output = self.ner_classifier(self.dropout(ner_attended))
        sentiment_output = self.sentiment_classifier(self.dropout(sentiment_attended.mean(dim=1)))
        
        ner_output += self.ner_residual(sequence_output)
        sentiment_output += self.sentiment_residual(pooled_output)
...
if __name__ == "__main__":
    ...
    model = BERTBiLSTMMultiHeadNERSentimentModel(num_ner_labels=5, num_sentiment_labels=3, num_lstm_layers=3)
    ...
    plot_confusion_matrix(
        ...
        title='BERT-BiLSTM-MultiHead Sentiment Analysis Confusion Matrix'
    )
    ...
    plot_metrics_comparison(
        ...
        title='BERT-BiLSTM-MultiHead Performance Comparison'
    )

    save_performance_metrics(
        "BERT_BiLSTM_MultiHead_NER_Sentiment",
        train_losses,
        val_losses,
        metrics,
    ...
    print("Performance metrics saved to 'performance_BERT_BiLSTM_MultiHead.json'")
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.title('BERT-BiLSTM-MultiHead Training Progress')
    plt.savefig('./performance_improved/training_progress_BERT_BiLSTM_MultiHead.png')
    plt.show()
