...
class AdvancedHierarchicalMultiHeadBERTBiLSTM(nn.Module):
    def __init__(self, num_ner_labels, num_sentiment_labels, hidden_size=256, num_heads=8, shared_lstm_layers=2, task_specific_lstm_layers=1):
        super(AdvancedHierarchicalMultiHeadBERTBiLSTM, self).__init__()
        ...
        self.shared_lstm = nn.LSTM(
            ...
            bidirectional=True,
            batch_first=True,
            dropout=0.1 if shared_lstm_layers > 1 else 0
        )
        
        # Task-specific BiLSTM layers
        self.ner_lstm = nn.LSTM(
            ...
            bidirectional=True,
            batch_first=True,
            dropout=0.1 if task_specific_lstm_layers > 1 else 0
        )
        ...
        self.ner_attention = nn.MultiheadAttention(hidden_size * 2, num_heads, dropout=0.1, batch_first=True)
        self.sentiment_attention = nn.MultiheadAttention(hidden_size * 2, num_heads, dropout=0.1, batch_first=True)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        ner_attention_output, _ = self.ner_attention(ner_lstm_output, ner_lstm_output, ner_lstm_output, key_padding_mask=attention_mask.eq(0))
        ner_output = self.ner_classifier(self.dropout(ner_attention_output))
        ner_output += self.ner_residual(sequence_output)  # Residual connection
        
        # Sentiment task
        sentiment_lstm_output, _ = self.sentiment_lstm(shared_lstm_output)
        sentiment_attention_output, _ = self.sentiment_attention(sentiment_lstm_output, sentiment_lstm_output, sentiment_lstm_output, key_padding_mask=attention_mask.eq(0))
        # Global average pooling for sentiment analysis
        sentiment_pooled = torch.mean(sentiment_attention_output, dim=1)
        sentiment_output = self.sentiment_classifier(self.dropout(sentiment_pooled))
        sentiment_output += self.sentiment_residual(pooled_output)  # Residual connection
...
if __name__ == "__main__":
    ...
    model = AdvancedHierarchicalMultiHeadBERTBiLSTM(
        num_ner_labels=5, 
    ...
    plot_confusion_matrix(
        ...
        title='MultiHead_Hierarchial Sentiment Analysis Confusion Matrix'
    )
    ...
    plot_metrics_comparison(
        ...
        title='MultiHead Hierarchial Performance Comparison'
    )
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.title('MultiHead_Hierarchial Training Progress')
    plt.savefig('./performance_improved/training_progress_MultiHead_Hierarchial.png')
    plt.show()
